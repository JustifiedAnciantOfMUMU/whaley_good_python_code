        ...
        for i in _dict:
            if os.path.exists(folder_path + '/audio/' + _dict[i]['file']):
                ...
                if _dict[i]['label'] == str(1) and i > 0:
                    Spectrogram().save_spectrogram(t, f, _denoised_spectrogram, (self.dataset_dir + '/call'), self._get_file_name(_dict[i]['file'], _dict[i]['label']))
                    #Spectrogram().save_spectrogram(t, f, _denoised_spectrogram, (self.dataset_dir + '/call'), file[:-4])
                else:
                    Spectrogram().save_spectrogram(t, f, _denoised_spectrogram, (self.dataset_dir + '/no_call'), self._get_file_name(_dict[i]['file'], _dict[i]['label']))
