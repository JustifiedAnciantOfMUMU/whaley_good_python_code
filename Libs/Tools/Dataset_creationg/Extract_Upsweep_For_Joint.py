    ...
    def _extract_folder(self, folder_path, snr_col=2, label_col=1):
        ...
        for i in _dict:
            if os.path.exists(folder_path + '/audio/' + _dict[i]['file']):
                f, t, _spectrogram = Spectrogram().create_spectrgram(folder_path + '/audio/' + _dict[i]['file'])
                print(f[-1])
                ...
                if _dict[i]['label'] == str(1) and i > 0:
                    Spectrogram().save_spectrogram(t, f, _denoised_spectrogram, (self.dataset_dir + '/call'), self._get_file_name(_dict[i]['file'], _dict[i]['label']), 0, 500)
                    #Spectrogram().save_spectrogram(t, f, _denoised_spectrogram, (self.dataset_dir + '/call'), file[:-4])
                else:
                    Spectrogram().save_spectrogram(t, f, _denoised_spectrogram, (self.dataset_dir + '/no_call'), self._get_file_name(_dict[i]['file'], _dict[i]['label']), 0, 500)
...
if __name__ == '__main__':
    ...
    database_kirsebom = KirsebomDatabase(config_dict['kirsebom_data'], r'E:\Thesis\Datasets\230904_Combined\Spectrograms\all')
